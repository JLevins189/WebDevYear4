...
function fillRepo(repoJson) {
  repoJson.forEach(function (currentRepo, index) {
    ...
    let currentRepoNameElement =
      document.getElementById(repoNameElementPrefix + (index + 1)) ??
      document.createElement("p");
    let currentRepoDescriptionElement =
      document.getElementById(repoDescriptionElementPrefix + (index + 1)) ??
      document.createElement("p");
...
function setElement(element, value) {
  ...
  if (value || value === 0) {
    //0 is marked as false otherwise
    ...
    let errorTextNode = document.createTextNode(
      "This information could not be found or is blank"
    );
