...
function validateEmail(email) {
  ...
  if (
    lastPositionOfAt < lastPositionOfDot &&
    lastPositionOfAt > 0 &&
    email.indexOf("@@") === -1 &&
    email.length - lastPositionOfAt > 2
  )
    return true;
  return false;
}
...
function clearForm() {
  addContactForm.querySelectorAll("input").forEach((element) => {
    if (!(element.type === "submit")) element.value = "";
  }); //clear all values except submit button
}

function hideNoContactsMessage() {
  // hide empty contacts message
  noContactsMessage.style.display = "none";
  //show header and table
  contactsContentDiv.style.display = "block";
}

function sortTable(table, rowNumber, reverse) {
  ...
  tr = tr.sort(function (a, b) {
    ...
    return (
      ...
      a.cells[rowNumber].textContent
        .trim() // using `.textContent.trim()` for test
        .localeCompare(b.cells[rowNumber].textContent.trim())
    );
  });
  for (let i = 0; i < tr.length; ++i) tb.appendChild(tr[i]); // append sorted rows
}
...
function filterResults() {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (
      mobileNumber.toUpperCase().indexOf(searchInput.value.toUpperCase()) > -1
    ) {
      //i.e. number is found as substring
